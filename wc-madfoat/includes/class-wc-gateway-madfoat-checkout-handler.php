...
require_once($includes_path. 'abstracts/abstract-wc-gateway-madfoat.php');

class WC_Gateway_Madfoat_Checkout_Handler
{
    public function __construct()
    {
        $this->madfoat_payment_gateway = new WC_Madfoat_Payment_Gateway();
        ...
        $this->subs_method = wc_gateway_madfoat()->settings->__get('subscription_method');
    }
    ...
    */
    public function process_payment($order_id)
    {
        ...
        if($this->payment_mode_woocomm == '10' && (!isset($_POST['madfoat_payment_token']) || trim($_POST['madfoat_payment_token']) == '')){
        ...
        $madfoat_ref = trim($result['order']['ref']);
        $madfoat_url = trim($result['order']['url']);

        if (empty($madfoat_ref) || empty($madfoat_url)) {
        ...
        update_post_meta( $order_id, '_madfoat_ref', $madfoat_ref);
        ...
        if(is_ssl() && ($this->payment_mode_woocomm == 2 || $this->payment_mode_woocomm == '9' || $this->payment_mode_woocomm == '10') && !isset($_POST['woocommerce_change_payment'])) {
            if (get_post_meta($order_id, '_madfoat_url')) {
                delete_post_meta( $order_id, '_madfoat_url');
            }
            add_post_meta( $order_id, '_madfoat_url', $madfoat_url);
            if($this->payment_mode_woocomm == '9' || $this->payment_mode_woocomm == '10'){
                return array(
                    'result'    => 'success',
                    'redirect'    => false,
                    'iframe_url'  => $madfoat_url,
                );
            }else{
                $madfoat_url = $order->get_checkout_payment_url(true);
            }
        }
        
        return array(
            'result'    => 'success',
            'redirect'  => $madfoat_url,
    ...
    */
    ...
    {
        $payment_url = get_post_meta($order_id, '_madfoat_url', true);
        $style = '#madfoat {width: 100%; min-width: 600px; height: 600px; border: none;}';
        $style .= ".order_details {display: none;}";
        echo "<style>$style</style>";
        echo ' <iframe id= "madfoat" src= "'.$payment_url.'"  sandbox="allow-forms allow-modals allow-popups-to-escape-sandbox allow-popups allow-scripts allow-top-navigation allow-same-origin"></iframe>';
    }
    ...
    */
    private function generate_request($order)
    {
       ...
       $prefix=$productnames="";
        ...
        $ivp_lang = wc_gateway_madfoat()->settings->__get('language');
        ...
        if (get_post_meta($order_id, '_madfoat_cartid')) {
            delete_post_meta($order_id, '_madfoat_cartid');
        }
        add_post_meta($order_id, '_madfoat_cartid', $cart_id);
        ...
        add_post_meta($order_id, '_madfoat_cartdesc', $cart_desc);
        ...
        $data = array(
            ...
            'ivp_authkey'     => wc_gateway_madfoat()->settings->__get('store_secret'),
            'ivp_cart'        => $cart_id,
            'ivp_test'        => $test_mode,
        ...
        if($this->payment_mode_woocomm == '10' && isset($_POST['madfoat_payment_token']) && trim($_POST['madfoat_payment_token']) != ''){
            ...
            $data['card_token'] = trim($_POST['madfoat_payment_token']);
        }
        ...
        if($this->subs_method == 'madfoat'){
            ...
            foreach ($order_items as $product_data) {
                ...
                $isSubProduct = get_post_meta($productId, '_subscription_madfoat', true);
                if($isSubProduct == 'yes'){
