<?php
/*
 * Madfoat plugin for woocommercee
*/
...
class WC_Gateway_Madfoat_Plugin
{
    ...
    */
    public function maybe_run()
    {
        ...
        $madfoatSettings = (array) get_option('woocommerce_wcmadfoat_settings', array());
        if($madfoatSettings['subscription_method'] == 'madfoat'){
            add_filter( 'product_type_options', array($this, 'subscription_product_option') );
            add_action( 'woocommerce_process_product_meta_simple', array($this, 'save_madfoat_subscription_option_fields')  );
            add_action( 'woocommerce_process_product_meta_variable', array($this, 'save_madfoat_subscription_option_fields')  );
    ...
    public function bootstrap()
    {
        try {
            ...
            $this->_run();
            delete_option('wc_gateway_madfoat_bootstrap_warning_message');
        } catch (Exception $e) {
            if (in_array($e->getCode(), array(self::DEPENDENCIES_UNSATISFIED))) {

                update_option('wc_gateway_madfoat_bootstrap_warning_message', $e->getMessage());
            }
    ...
    protected function _check_dependencies()
    {
        if (!function_exists('WC')) {
            throw new Exception(__('Madfoat Secure payments for WooCommerce requires WooCommerce to be activated', 'madfoat-for-woocommerce'), self::DEPENDENCIES_UNSATISFIED);
        }

        if (version_compare(WC()->version, '3.0', '<')) {
            throw new Exception(__('Madfoat Secure payments for WooCommerce requires WooCommerce version 3.0 or greater', 'madfoat-for-woocommerce'), self::DEPENDENCIES_UNSATISFIED);
        }

        if (!function_exists('curl_init')) {
            throw new Exception(__('Madfoat Secure payments for WooCommerce requires cURL to be installed on your server', 'madfoat-for-woocommerce'), self::DEPENDENCIES_UNSATISFIED);
        }
    }

    function subscription_product_option( $product_type_options ) {
        global $post;
        $isSubscriptionOn = get_post_meta($post->ID, '_subscription_madfoat', true);

        $product_type_options['Recurring Enabled'] = array(
            'id'            => '_subscription_madfoat',
    ...
    function save_madfoat_subscription_option_fields( $post_id ) {
        $is_madfoat_subscription = isset( $_POST['_subscription_madfoat'] ) ? 'yes' : 'no';
        if( !empty( $is_madfoat_subscription ) )
            update_post_meta( $post_id, '_subscription_madfoat', esc_attr( $is_madfoat_subscription ) );
        else {
            update_post_meta( $post_id, '_subscription_madfoat',  'no' );
        }
    }
    ...
    public function madfoat_process_cont_auth() {
        global $wpdb;

        $test_mode  = (wc_gateway_madfoat()->settings->__get('testmode') == 'yes') ? 1 : 0;
        $table_billing = $wpdb->prefix . 'madfoat_repeat_billing';
        $table_billing_txn = $wpdb->prefix . 'madfoat_repeat_billing_transactions';
        ...
        foreach ($todayBilling as $billingRow) {
            extract($billingRow);
            // Available variables: id, order_id, cart_id, product_id, cart_desc, madfoat_txnref, initial_payment, recurring_amount, 
            ...
            if($txnAmount > 0){
                ...
                $response = wc_gateway_madfoat()->checkout->remote_api_request($data);
                ...
                $insData = $wpdb->insert( 
                    $table_billing_txn, 
                      array( 
                        ...
                        'madfoat_txnref_orig' => $madfoat_txnref,
                        'madfoat_txnref' => $txnRef,
        ...
        echo "Madfoat Recurring Cron Run for " . count($todayBilling) . " transactions. on " . date("Y-m-d H:i:s") . "\n";
        exit;
    }

    public function show_bootstrap_warning()
    {
        $dependencies_message = get_option('wc_gateway_madfoat_bootstrap_warning_message', '');
    ...
    protected function _load_handlers()
    {
        ...
        // Load handlers.
        require_once($this->includes_path . 'class-wc-gateway-madfoat-settings.php');
        require_once($this->includes_path . 'class-wc-gateway-madfoat-gateway-loader.php');
        require_once($this->includes_path . 'class-wc-gateway-madfoat-admin-handler.php');
        require_once($this->includes_path . 'class-wc-gateway-madfoat-checkout-handler.php');

        $this->settings       = new WC_Gateway_Madfoat_Settings();
        $this->gateway_loader = new WC_Gateway_madfoat_Gateway_Loader();
        $this->admin          = new WC_Gateway_Madfoat_Admin_Handler();
        $this->checkout       = new WC_Gateway_Madfoat_Checkout_Handler();
    }
    ...
    public function plugin_action_links($links)
    {
        ...
        $plugin_links[] = '<a href="' . esc_url($setting_url) . '">' . esc_html__('Settings', 'wcmadfoat') . '</a>';
    ...
    {
        return admin_url('admin.php?page=wc-settings&tab=checkout&section=wcmadfoat');
    }
